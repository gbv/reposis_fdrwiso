...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="mcri18n mir mods xlink">
    ...
    <xsl:template match="/">
      ...
      <xsl:if test="
        $mods/mods:extension[@type='temporal_period_of_reference']/mir:subject[@type='period_of_reference']
      ">
        <xsl:variable name="period_of_reference" select="
          $mods/mods:extension[@type='temporal_period_of_reference']/mir:subject[@type='period_of_reference']
        " />
          ...
          <xsl:if test="$period_of_reference/mods:temporal[@encoding='w3cdtf'][@point='start']">
            <xsl:variable name="formatStartDate">
              <xsl:choose>
                <xsl:when test="
                  string-length(
                    normalize-space($period_of_reference/mods:temporal[@encoding='w3cdtf'][@point='start'])
                  )=4
                ">
            ...
            <xsl:call-template name="formatISODate">
              <xsl:with-param name="date" select="
                $period_of_reference/mods:temporal[@encoding='w3cdtf'][@point='start']
              " />
          ...
          <xsl:if test="$period_of_reference/mods:temporal[@encoding='w3cdtf'][@point='end']">
            <xsl:variable name="formatEndDate">
              <xsl:choose>
                <xsl:when test="
                  string-length(
                    normalize-space($period_of_reference/mods:temporal[@encoding='w3cdtf'][@point='end'])
                  )=4
                ">
                  <xsl:value-of select="'yyyy'" />
                </xsl:when>
            ...
            <xsl:call-template name="formatISODate">
              <xsl:with-param name="date" select="
                $period_of_reference/mods:temporal[@encoding='w3cdtf'][@point='end']
              " />
        ...
        <div id="fdrwiso-metadata">
          <div class="mir_metadata">
            <dl>
              <dt>
                <xsl:value-of select="concat(mcri18n:translate('component.mods.metaData.dictionary.period'), ': ')" />
              </dt>
              <dd>
