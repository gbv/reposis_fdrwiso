        ...
        <ddi:stdyInfo>
          <ddi:subject>
            <xsl:variable name="jel">
              <xsl:value-of
                select="substring-after(//mods:classification[@displayLabel='jel']/@valueURI, '#')" />
            </xsl:variable>
            <!-- TODO use WebApplicationBaseURL -->
            <ddi:keyword
              xml:lang="en"
              vocab="JEL"
              vocabURI="https://emporion.gswg.info/api/v2/classifications/jel"
              ID="{$jel}">
